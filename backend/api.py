    ...
    try:
        ...
        for device in devices:
            ...
            if not existing:
                ...
                for alert in alerts:
                    if detection_engine.deduplicate_alerts(existing.id if existing else 0, alert["rule_id"]):
                        db.add_alert(existing.id if existing else 1, alert["rule_id"], alert["severity"], alert["description"])
            else:
